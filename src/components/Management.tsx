    ...
    <div className="flex-1 overflow-auto bg-slate-950 p-4 sm:p-8 text-slate-100 pb-24">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1">
              Currently Managing: <strong className="text-white">{(hotels || []).length} Hotels</strong> across <strong className="text-white">{allBrands.length} Brands</strong>.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Bed Capacity</p>
            <p className="text-lg sm:text-2xl font-black text-white mt-0.5">{totalBeds * 2} <span className="text-xs text-slate-400">Guests</span></p>
          </div>
          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Floors Built</p>
            <p className="text-lg sm:text-2xl font-black text-white mt-0.5">{floors.length} Levels</p>
          </div>
          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Active Staff</p>
            <p className="text-lg sm:text-2xl font-black text-white mt-0.5">{staff.length} Employees</p>
          </div>
        </div>
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
            ...
            <div className="p-5">
              <div className="mb-6 p-4 border border-violet-900/30 bg-violet-950/10 rounded-xl">
                ...
                <p className="text-xs text-violet-400 mb-3">Describe your ideal theme below. The AI architect will draft a functional layout grid complete with beds, reception desks, tables, and walls.</p>
                <form 
                  onSubmit={async (e) => {
                    e.preventDefault();
                      ...
                      if (data.floors && Array.isArray(data.floors)) {
                        useHotelStore.setState(() => {
                          const charToTile: Record<string, any> = {
                            ...
                            'T': 'table', 'E': 'elevator', 'X': 'stairs'
                          };
                          const parsedFloors = data.floors.map((f: any) => {
                            const grid = f.grid.map((row: string) => 
                          ...
                          return { floors: parsedFloors, activeFloorIndex: 0 };
                        });
                        alert("AI Architect has generated your hotel layers successfully!");
                      } else if (data.grid) {
                ...
                >
                  <input 
                    type="text" 
                    name="prompt"
                    placeholder="e.g. A tiny seaside bed and breakfast" 
