...
Built and managed with ArchHotel Suite!`;
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-slate-950 text-slate-100 font-sans overflow-hidden pb-16 sm:pb-0">
      <nav className="h-14 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-4 sm:px-6 shrink-0 shadow-md z-20">
        <div className="flex items-center gap-4 sm:gap-8">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-8 h-8 bg-amber-500 rounded-lg flex items-center justify-center shadow-lg shadow-amber-500/10 shrink-0">
              <span className="text-slate-950 font-black text-sm">{activeBrand.icon}</span>
            </div>
            <div className="flex flex-col">
              <span className="font-extrabold tracking-tight text-white leading-none text-xs sm:text-sm">
                {activeHotel.name} <span className="text-[10px] sm:text-xs font-normal text-amber-500 block sm:inline mt-0.5 sm:mt-0 sm:ml-1 font-mono uppercase">[{activeBrand.name}]</span>
              </span>
              {/* Stars display */}
